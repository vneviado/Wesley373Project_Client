  ...
  def initialize(user)
    ...
    if user.admin?
        ...
        can :manage, Service do |s|
            users_in_service = user.services.split(",")
            users_in_service.include? s.id
        end

        can :manage, ServiceType do |st|
            user.services.split(",").include? st.service.id
        end

        can :manage, ServiceLocation do |sl|
            user.services.split(",").include? sl.service.id
        end

        can :manage, ServiceContact do |sc|
            user.services.split(",").include? sc.service.id
        end
